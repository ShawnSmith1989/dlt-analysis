 ...
 * 
 * 开发说明：
 * - 2026-03-21: 初始开发，实现五行映射和号码生成算法
 * - 包含时辰、天气、地理、生肖、星座等参数的五行映射
 * - 核心算法：calculateElementScore() 和 generateNumbers()
 */
...
(function() {
    ...
    const DREAM_MAP = {
        'water': { name: '梦见水', element: '水' },
        'fire': { name: '梦见火', element: '火' },
        'wood': { name: '梦见树木', element: '木' },
        'metal': { name: '梦见金属', element: '金' },
        'earth': { name: '梦见土地', element: '土' },
        'flying': { name: '梦见飞行', element: '火' },
        'falling': { name: '梦见坠落', element: '水' },
        'chasing': { name: '梦见追逐', element: '木' }
    };
    ...
    function calculateElementScore(inputs) {
        ...
        if (inputs.dreamType && DREAM_MAP[inputs.dreamType]) {
            const element = DREAM_MAP[inputs.dreamType].element;
            scores[element] += WEIGHTS.dreamType;
            totalWeight += WEIGHTS.dreamType;
        }
        
        if (inputs.intuitionNumber !== null && inputs.intuitionNumber !== undefined && inputs.intuitionNumber !== '') {
            const element = getNumberElement(inputs.intuitionNumber);
            scores[element] += WEIGHTS.intuitionNumber;
            totalWeight += WEIGHTS.intuitionNumber;
        }
    ...
    function getNumberColor(num, isBackArea = false) {
        if (isBackArea) {
            if (num <= 3) return ELEMENT_COLORS['水'];
            if (num <= 6) return ELEMENT_COLORS['金'];
            if (num <= 9) return ELEMENT_COLORS['火'];
            if (num === 10) return ELEMENT_COLORS['土'];
            return ELEMENT_COLORS['木'];
        }
        
        if (num <= 7) return ELEMENT_COLORS['水'];
        if (num <= 14) return ELEMENT_COLORS['金'];
        if (num <= 21) return ELEMENT_COLORS['火'];
        if (num <= 28) return ELEMENT_COLORS['土'];
        return ELEMENT_COLORS['木'];
    }
    ...
    function renderResult(result) {
        ...
        result.backNumbers.forEach((num, index) => {
            const ball = document.createElement('span');
            ball.className = 'result-ball';
            ball.style.backgroundColor = getNumberColor(num, true);
            ball.textContent = formatNumber(num);
        ...
        result.frontNumbers.forEach(num => {
            if (num <= 7) elementCounts['水']++;
            else if (num <= 14) elementCounts['金']++;
            else if (num <= 21) elementCounts['火']++;
            else if (num <= 28) elementCounts['土']++;
        ...
        Object.entries(elementCounts).forEach(([element, count]) => {
            if (count > 0) {
                ...
                badge.innerHTML = `<span class="dot" style="background-color: ${ELEMENT_COLORS[element]}"></span>${element}(${count})`;
    ...
    function collectInputs() {
        return {
            ...
            dreamType: document.getElementById('dreamType').value,
            intuitionNumber: document.getElementById('intuitionNumber').value
        };
    }
    ...
    function updateShichenDisplay() {
        ...
        if (!isNaN(hour) && hour >= 0 && hour <= 23) {
            ...
            shichenElement.style.backgroundColor = ELEMENT_COLORS[shichen.element];
            shichenElement.style.color = 'white';
        } else {
            shichenText.textContent = '--';
            shichenElement.textContent = '--';
            shichenElement.style.backgroundColor = '';
    ...
    function updateLocationDisplay() {
        ...
        if (province && PROVINCE_MAP[province]) {
            ...
            locationElementText.innerHTML = `<span class="badge" style="background-color: ${ELEMENT_COLORS[element]}; color: white;">${element}</span>`;
        } else {
    ...
    function showNotification(message, type = 'info') {
        ...
        notification.style.cssText = 'top: 20px; left: 50%; transform: translateX(-50%); z-index: 9999; min-width: 250px;';
    ...
    function applyDefaultValues() {
        ...
        document.getElementById('dreamType').value = '';
        document.getElementById('intuitionNumber').value = '';
